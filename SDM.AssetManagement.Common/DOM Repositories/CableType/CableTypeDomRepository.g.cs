...
namespace Skyline.DataMiner.SDM.AssetManagement.Models
{
    ...
    internal partial class CableTypeDomRepository : IBulkRepository<CableType>
    {
        ...
        public IReadOnlyCollection<CableType> Create(IEnumerable<CableType> createObjects)
        {
            if (createObjects is null || !createObjects.Any())
            {
                return Array.Empty<CableType>();
            }
            ...
            {
                existing.UnionWith(Read(new ORFilterElement<CableType>(batch.Select(obj => CableTypeExposers.Identifier.Equal(obj.Identifier)).ToArray())).Select(obj => obj.Identifier));
            }

            // Create the remainder
            var SuccessfulItems = new List<CableType>();
            ...
            var exceptionBuilder = new SdmBulkCrudException<CableType>.Builder();
            foreach (var obj in createObjects)
            {
                if (existing.Contains(obj.Identifier))
                {
                    exceptionBuilder.AddFailed(obj, new SdmCrudException<CableType>(obj, $"Could not create CableType with guid: '{obj.Identifier}', it already exists."));
                    continue;
                }
        ...
        public IReadOnlyCollection<CableType> CreateOrUpdate(IEnumerable<CableType> items)
        {
            ...
            var successful = new List<CableType>();
            var exceptionBuilder = new SdmBulkCrudException<CableType>.Builder();
        ...
        public long Count(FilterElement<CableType> filter)
        {
            if (filter is null)
            {
        ...
        public long Count(IQuery<CableType> query)
        {
            if (query is null)
            {
        ...
        public IEnumerable<CableType> Read(FilterElement<CableType> filter)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
        ...
        public IEnumerable<CableType> Read(IQuery<CableType> query)
        {
            if (query is null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        public IEnumerable<IPagedResult<CableType>> ReadPaged(FilterElement<CableType> filter)
        {
            return ReadPaged(filter, 500);
        }

        public IEnumerable<IPagedResult<CableType>> ReadPaged(FilterElement<CableType> filter, int pageSize)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
            ...
            while (moveNext)
            {
                ...
                var result = new PagedResult<CableType>(page, i, pageSize, moveNext);
                yield return result;
                i++;
            }
        }

        public IEnumerable<IPagedResult<CableType>> ReadPaged(IQuery<CableType> query)
        {
            return ReadPaged(query, 500);
        }

        public IEnumerable<IPagedResult<CableType>> ReadPaged(IQuery<CableType> query, int pageSize)
        {
            if (query is null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        public CableType Update(CableType updateObject)
        {
        ...
        public IReadOnlyCollection<CableType> Update(IEnumerable<CableType> updateObjects)
        {
            ...
            foreach (var obj in updateObjects)
            {
                if (!existing.Contains(obj.Identifier))
                {
                    exceptionBuilder.AddFailed(obj, new SdmCrudException<CableType>(obj, "Could not update a non existing CableType"));
                    continue;
                }
        ...
        public void Delete(CableType deleteObject)
        {
        ...
        public void Delete(IEnumerable<CableType> deleteObjects)
        {
        ...
        private IEnumerable<CableType> Read(FilterElement<DomInstance> domFilter)
        {
        ...
        private IEnumerable<CableType> Read(IQuery<DomInstance> domQuery)
        {
            if (domQuery is null)
            {
        ...
        private IEnumerable<IEnumerable<CableType>> ReadPaged(FilterElement<DomInstance> domFilter, int pageSize)
        {
            if (domFilter is null)
            {
                throw new ArgumentNullException(nameof(domFilter));
        ...
        private IEnumerable<IEnumerable<CableType>> ReadPaged(IQuery<DomInstance> domQuery, int pageSize)
        {
            if (domQuery is null)
            {
                throw new ArgumentNullException(nameof(domQuery));
        ...
        private FilterElement<DomInstance> TranslateFullFilter(FilterElement<CableType> filter)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
        ...
        private CableType FromInstance(DomInstance instance)
        {
            var obj = new CableType
            {
        ...
        private DomInstance ToInstance(CableType obj)
        {
