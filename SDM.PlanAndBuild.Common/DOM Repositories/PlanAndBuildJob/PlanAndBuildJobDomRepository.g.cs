    ...
    internal partial class PlanAndBuildJobDomRepository : IBulkRepository<PlanAndBuildJob>
	{
		...
		private PlanAndBuildJob FromInstance(DomInstance instance)
		{
			...
			if (_ownershipSection != default)
			{
				...
				var _ownershipassignedto = _ownershipSection.GetValue<Guid>(PlanAndBuild.Models.PlanAndBuildJobDomMapper.Ownership.AssignedTo);
				if (_ownershipassignedto != null)
				{
					obj.Ownership.AssignedTo = new PnoObjectReference<Person>(_ownershipassignedto.Value);
				}

				var _ownershipassignmentgroup = _ownershipSection.GetValue<Guid>(PlanAndBuild.Models.PlanAndBuildJobDomMapper.Ownership.AssignmentGroup);
				if (_ownershipassignmentgroup != null)
				{
					obj.Ownership.AssignmentGroup = new PnoObjectReference<Team>(_ownershipassignmentgroup.Value);
				}
			}
		...
		private DomInstance ToInstance(PlanAndBuildJob obj)
		{
			...
			foreach (var attachments in obj.Attachments)
			{
				...
				{
					_attachmentsSection.AddOrUpdateValue<string>(PlanAndBuild.Models.PlanAndBuildJobDomMapper.Attachments.AttachedBy, attachments.AttachedBy);
				}
