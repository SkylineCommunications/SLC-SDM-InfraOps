...
namespace Skyline.DataMiner.SDM.PlanAndBuild.Models
{
    ...
    internal partial class JobTypeDomRepository : IBulkRepository<JobType>
    {
        ...
        public IReadOnlyCollection<JobType> Create(IEnumerable<JobType> createObjects)
        {
            if (createObjects is null || !createObjects.Any())
            {
                return Array.Empty<JobType>();
            }
            ...
            {
                existing.UnionWith(Read(new ORFilterElement<JobType>(batch.Select(obj => JobTypeExposers.Identifier.Equal(obj.Identifier)).ToArray())).Select(obj => obj.Identifier));
            }

            // Create the remainder
            var successfulItems = new List<JobType>();
            ...
            var exceptionBuilder = new SdmBulkCrudException<JobType>.Builder();
            foreach (var obj in createObjects)
            {
                if (existing.Contains(obj.Identifier))
                {
                    exceptionBuilder.AddFailed(obj, new SdmCrudException<JobType>(obj, $"Could not create JobType with guid: '{obj.Identifier}', it already exists."));
                    continue;
                }
        ...
        public IReadOnlyCollection<JobType> CreateOrUpdate(IEnumerable<JobType> items)
        {
            ...
            var successful = new List<JobType>();
            var exceptionBuilder = new SdmBulkCrudException<JobType>.Builder();
        ...
        public long Count(FilterElement<JobType> filter)
        {
            if (filter is null)
            {
        ...
        public long Count(IQuery<JobType> query)
        {
            if (query is null)
            {
        ...
        public IEnumerable<JobType> Read(FilterElement<JobType> filter)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
        ...
        public IEnumerable<JobType> Read(IQuery<JobType> query)
        {
            if (query is null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        public IEnumerable<IPagedResult<JobType>> ReadPaged(FilterElement<JobType> filter)
        {
            return ReadPaged(filter, 500);
        }

        public IEnumerable<IPagedResult<JobType>> ReadPaged(FilterElement<JobType> filter, int pageSize)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
            ...
            while (moveNext)
            {
                ...
                var result = new PagedResult<JobType>(page, i, pageSize, moveNext);
                yield return result;
                i++;
            }
        }

        public IEnumerable<IPagedResult<JobType>> ReadPaged(IQuery<JobType> query)
        {
            return ReadPaged(query, 500);
        }

        public IEnumerable<IPagedResult<JobType>> ReadPaged(IQuery<JobType> query, int pageSize)
        {
            if (query is null)
            {
                throw new ArgumentNullException(nameof(query));
        ...
        public JobType Update(JobType updateObject)
        {
        ...
        public IReadOnlyCollection<JobType> Update(IEnumerable<JobType> updateObjects)
        {
            ...
            foreach (var obj in updateObjects)
            {
                if (!existing.Contains(obj.Identifier))
                {
                    exceptionBuilder.AddFailed(obj, new SdmCrudException<JobType>(obj, "Could not update a non existing JobType"));
                    continue;
                }
        ...
        public void Delete(JobType deleteObject)
        {
        ...
        public void Delete(IEnumerable<JobType> deleteObjects)
        {
        ...
        private IEnumerable<JobType> Read(FilterElement<DomInstance> domFilter)
        {
        ...
        private IEnumerable<JobType> Read(IQuery<DomInstance> domQuery)
        {
            if (domQuery is null)
            {
        ...
        private IEnumerable<IEnumerable<JobType>> ReadPaged(FilterElement<DomInstance> domFilter, int pageSize)
        {
            if (domFilter is null)
            {
                throw new ArgumentNullException(nameof(domFilter));
        ...
        private IEnumerable<IEnumerable<JobType>> ReadPaged(IQuery<DomInstance> domQuery, int pageSize)
        {
            if (domQuery is null)
            {
                throw new ArgumentNullException(nameof(domQuery));
        ...
        private FilterElement<DomInstance> TranslateFullFilter(FilterElement<JobType> filter)
        {
            if (filter is null)
            {
                throw new ArgumentNullException(nameof(filter));
        ...
        private JobType FromInstance(DomInstance instance)
        {
            var obj = new JobType
            {
        ...
        private DomInstance ToInstance(JobType obj)
        {
