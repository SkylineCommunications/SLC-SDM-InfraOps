namespace SDM.AssetManagement.Tests.Models
{
    ...
    /// Skyline.DataMiner.SDM.AssetManagement.Models namespace. Guards against regressions such
    /// as calling GetHashCode() on a default (null-Identifier) SdmObjectReference&lt;T&gt; or
    /// ISdmObjectReference&lt;T&gt; property.
    /// </summary>
    [TestClass]
    public class ModelHashCodeTests
    {
        ...
        {
            return typeof(Asset).Assembly
                .GetTypes()
                .Where(t => t.IsClass
                    && t.IsPublic
                    && !t.IsAbstract
                    && !t.IsGenericTypeDefinition
                    && t.Namespace == "Skyline.DataMiner.SDM.AssetManagement.Models"
            ...
            return modelType
                ...
                .Where(p => p.CanRead
                    ...
                    && (p.PropertyType.GetGenericTypeDefinition() == typeof(SdmObjectReference<>)
                        || p.PropertyType.GetGenericTypeDefinition() == typeof(ISdmObjectReference<>)));
        }
    }
}
