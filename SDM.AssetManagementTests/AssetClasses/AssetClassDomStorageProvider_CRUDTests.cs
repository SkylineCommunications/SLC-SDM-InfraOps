	...
	public partial class AssetClassDomStorageProvider
	{
			...
			referenceAssetClass = new AssetClass
			{
				...
				PowerPorts = new List<PowerPortInfo>
				{
					...
					new PowerPortInfo
					{
						...
						Name = "Power Port 2",
						PortNumber = 2,
						OutputType = SlcAssetManagement.Enums.Outputtype.In,
		...
		public void AssetClassDomStorageProvider_EmptyDOM_CreateOrUpdate_Update()
		{
			...
			var updatedAssetClass = new AssetClass
			{
				...
				PowerSupply = SlcAssetManagement.Enums.PowerSupply.DC,
				DataPorts = new List<DataPortInfo>
				{
					new DataPortInfo
					{
		...
		private static void AssertAssetClassUpdateDifferences(AssetClass original, AssetClass updated)
		{
			using (new AssertionScope())
			{
				...
				updated.PowerSupply.Should().Be(SlcAssetManagement.Enums.PowerSupply.DC);
				...
				updated.DataPorts[0].Type.Should().BeEmpty();
				updated.DataPorts[0].PortExposure.Should().Be(SlcAssetManagement.Enums.PortExposure.Back);
				updated.DataPorts[0].OutputType.Should().Be(SlcAssetManagement.Enums.Outputtype.In);
		...
		private void AssertCreated(IAssetManagementApiHelper helper)
		{
			using (new AssertionScope())
			{
				...
				createdClass.PowerSupply.Should().Be(SlcAssetManagement.Enums.PowerSupply.AC);
				...
				createdClass.DataPorts[0].Type.Should().BeEmpty();
				createdClass.DataPorts[0].Label.Should().Be("Label1");
				createdClass.DataPorts[0].PortExposure.Should().Be(SlcAssetManagement.Enums.PortExposure.Front);
				createdClass.DataPorts[0].OutputType.Should().Be(SlcAssetManagement.Enums.Outputtype.Out);
				createdClass.PowerPorts.Should().HaveCount(2);
				createdClass.PowerPorts[0].PortNumber.Should().Be(1);
				createdClass.PowerPorts[0].PortExposure.Should().Be(SlcAssetManagement.Enums.PortExposure.Front);
				createdClass.PowerPorts[0].OutputType.Should().Be(SlcAssetManagement.Enums.Outputtype.Out);
				createdClass.PowerPorts[1].PortNumber.Should().Be(2);
				createdClass.PowerPorts[1].PortExposure.Should().Be(SlcAssetManagement.Enums.PortExposure.Back);
				createdClass.PowerPorts[1].OutputType.Should().Be(SlcAssetManagement.Enums.Outputtype.In);
				createdClass.Holders.Should().HaveCount(3);
				createdClass.Holders[0].HierarchyRole.Should().Be(SlcAssetManagement.Enums.HierarchyRole.Card);
				createdClass.Holders[0].SlotNumber.Should().Be(1);
				createdClass.Holders[1].HierarchyRole.Should().Be(SlcAssetManagement.Enums.HierarchyRole.Fan);
				createdClass.Holders[1].SlotNumber.Should().Be(2);
				createdClass.Holders[2].HierarchyRole.Should().Be(SlcAssetManagement.Enums.HierarchyRole.Chassis);
